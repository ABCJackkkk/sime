...
class LocationFrequencyTracker {
  ...
  // 暴露给存档恢复用
  List<LocationFrequencyRecord> get recentHistory => List.unmodifiable(_history);
  Set<String> get knownLocations => Set.unmodifiable(_knownFrequentLocations);
  ...
  /// 存档恢复：直接标记常去地点
  void markFrequent(String key) => _knownFrequentLocations.add(key);

  /// 存档恢复：清空内部状态
  void clear() {
    _history.clear();
    _knownFrequentLocations.clear();
  }
