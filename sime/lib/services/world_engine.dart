...
import 'package:sime/services/character_schedule.dart';
import 'package:sime/services/inter_character_relationship.dart';
import 'package:sime/services/information_propagation.dart';
import 'package:sime/services/calendar_service.dart';
import 'package:sime/services/location_frequency_tracker.dart';
...
class WorldEngine {
  ...
  Map<String, dynamic> toJson() => {
        ...
        'seasonDayCounter': _seasonDayCounter,
        'narrativeHistory': _narrativeHistory,
        'interCharRel': interCharRel.toJson(),
        'infoProp': infoProp.toJsonList(),
        'locationTracker': locationTracker.toJson(),
      };

  void loadFromJson(Map<String, dynamic> json) {
    _currentDay = json['currentDay'] as int? ?? _currentDay;
    _phaseIndex = json['phaseIndex'] as int? ?? 0;
    _currentWeather = json['currentWeather'] as String? ?? _currentWeather;
    _currentSeason = json['currentSeason'] as String? ?? _currentSeason;
    _seasonDayCounter = json['seasonDayCounter'] as int? ?? 0;
    _narrativeHistory = json['narrativeHistory'] as String? ?? _narrativeHistory;
    calendar.currentDay = _currentDay;
    final inter = json['interCharRel'] as Map<String, dynamic>?;
    if (inter != null) interCharRel.fromJson(inter);
    final info = json['infoProp'] as List<dynamic>?;
    if (info != null) infoProp.fromJsonList(info);
    // locationTracker 是 final 字段，需要重建内部状态
    final loc = json['locationTracker'] as Map<String, dynamic>?;
    if (loc != null) {
      final restored = LocationFrequencyTracker.fromJson(loc);
      // 通过内部重建：清空后逐条 replay
      locationTracker.clear();
      for (final r in restored.recentHistory) {
        locationTracker.record(r.charId, r.locationId, r.day);
      }
      for (final k in restored.knownLocations) {
        locationTracker.markFrequent(k);
      }
    }
  }
}
