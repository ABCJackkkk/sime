...
class SimProfileScreen extends StatelessWidget {
            ...
            return SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  _buildCurrencyCard(app),
                  const SizedBox(height: 12),
                  _buildAffectionSection(app),
                  const SizedBox(height: 12),
                  _buildInventorySection(app),
  ...
  Widget _buildCurrencyCard(AppProvider app) {
    final gold = app.currencies['gold'] ?? 0;
    return GlassContainer(
      padding: const EdgeInsets.all(16),
      child: Row(children: [
        Container(width: 36, height: 36, decoration: BoxDecoration(borderRadius: BorderRadius.circular(10), color: AppColors.warning.withAlpha(25)), child: const Icon(CupertinoIcons.money_dollar_circle_fill, size: 20, color: AppColors.warning)),
        const SizedBox(width: 12),
        const Text('金币', style: TextStyle(fontSize: 15, color: AppColors.textSecondary)),
        const Spacer(),
        Text('$gold', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w700, color: AppColors.warning)),
      ]),
    );
  }
  ...
  Widget _buildInventorySection(AppProvider app) {
    final script = app.script;
    final ownedItems = <ShopItem>[];
    if (script != null) {
      for (final item in [...script.items.gifts, ...script.items.shopItems]) {
        if (app.hasItem(item.id)) ownedItems.add(item);
      }
    }
    return GlassContainer(
      padding: const EdgeInsets.all(16),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(width: 28, height: 28, decoration: BoxDecoration(borderRadius: BorderRadius.circular(7), color: AppColors.success.withAlpha(25)), child: const Icon(CupertinoIcons.bag_fill, size: 14, color: AppColors.success)),
          const SizedBox(width: 10),
          const Text('背包', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: AppColors.textPrimaryDark)),
          const Spacer(),
          Text('${ownedItems.length}件', style: const TextStyle(fontSize: 12, color: AppColors.textTertiary)),
        ]),
        const SizedBox(height: 12),
        if (ownedItems.isEmpty)
          const Text('背包空空，去商店购买吧', style: TextStyle(fontSize: 13, color: AppColors.textTertiary))
        else
          Wrap(
            spacing: 8,
            runSpacing: 8,
            children: ownedItems.map((item) => Container(
              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 7),
              decoration: BoxDecoration(borderRadius: BorderRadius.circular(20), color: AppColors.accent.withAlpha(15), border: Border.all(color: AppColors.accent.withAlpha(40), width: 0.5)),
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                const Icon(CupertinoIcons.gift_fill, size: 12, color: AppColors.accent),
                const SizedBox(width: 5),
                Text(item.name, style: const TextStyle(fontSize: 12, color: AppColors.accent)),
              ]),
            )).toList(),
          ),
      ]),
